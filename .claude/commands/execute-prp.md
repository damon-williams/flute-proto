# Execute BASE PRP with Archon Task Management
...
## Step 1: Read and Parse the PRP File: $ARGUMENTS
...
- The plan file will contain a list of tasks to implement, references to existing codebase components and integration points, and context about where to look in the codebase for implementation
...
Launch the validator agent using the Task tool
...
When validation fails, use the patterns and gotchas from the PRP to fix issues, then re-run validation until passing.
